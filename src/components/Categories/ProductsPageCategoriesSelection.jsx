...
const ProductsPageCategories = () => {
  ...
  const handleCategorySelection = async (evt) =>{
    let { category } = evt?.target?.dataset;
    const { id } = evt?.target;
    ...
    async function handleCategoryFilter(category, id) {
      try {
        ...
        if(categorySelected[category] === id) {
          ...
          setCategorySelected(data => ({ ...data, [category]: id }));
        }
      } catch (err) {
        console.error("handleCategorySelection: problem_loading", err);
        ...
        <form 
          ...
          {categories.map(({category, id}) => ( 
            <label key={id} htmlFor={category.toLowerCase()} className="flex items-center justify-start gap-2 cursor-pointer group hover:text-blue">
              <input className="not-sr-only" id={id} data-category={category} type="checkbox" />
